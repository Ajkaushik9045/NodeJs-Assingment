...
class SchoolController {
  ...
  try {
    ...
    // Validation checks (same as your code)
    ...
    if (latitude < -90 || latitude > 90) {
      return res.status(400).json({ status: "error", message: "Latitude must be between -90 and 90 degrees" });
    }
    if (longitude < -180 || longitude > 180) {
      return res.status(400).json({ status: "error", message: "Longitude must be between -180 and 180 degrees" });
    }

    if (typeof name !== "string" || typeof address !== "string") {
      return res.status(400).json({ status: "error", message: "Name and address must be strings" });
    }
    if (typeof latitude !== "number" || typeof longitude !== "number") {
      return res.status(400).json({ status: "error", message: "Latitude and longitude must be numbers" });
    }

    // Try to add the school
